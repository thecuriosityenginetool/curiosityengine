...
-- Step 1: Check if organization_integrations table exists and has correct structure
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'organization_integrations') THEN
        RAISE NOTICE '❌ organization_integrations table does not exist';
    ELSE
        RAISE NOTICE '✅ organization_integrations table exists';
    END IF;
END $$;
...
-- Step 5: Create organization if missing
DO $$
DECLARE
    v_user_id uuid;
    v_org_id uuid;
...
RAISE NOTICE '========================================';
RAISE NOTICE '1. If organization was just created, the Microsoft OAuth should now work';
RAISE NOTICE '2. Try connecting Outlook from Dashboard → Integrations';
RAISE NOTICE '3. Check browser console for detailed logs';
RAISE NOTICE '4. Both Microsoft login AND Outlook integration should work';
RAISE NOTICE '';
