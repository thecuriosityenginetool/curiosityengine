  ...
  try {
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        new URL('/dashboard?error=Invalid Monday.com callback', process.env.NEXT_PUBLIC_APP_URL)
      );
    }
    ...
    return NextResponse.redirect(
      new URL('/dashboard?success=Monday.com connected successfully', process.env.NEXT_PUBLIC_APP_URL)
    );
  } catch (error) {
    console.error('Error handling Monday.com callback:', error);
    return NextResponse.redirect(
